...
app.post('/api/register', async (req, res) => {
    ...
    try {
        ...
        const customer = new Customer({
            title, name, username, email, password: hashedPassword,
            phone, dob, address, street, city, zip, state, country
        });
        await customer.save();
        res.status(201).json({ message: 'User registered successfully' });
    } catch (err) {
...
app.post('/api/login', async (req, res) => {
    ...
    try {
        ...
        const token = jwt.sign({ userId: user._id, fullName: user.name, role: user.role || 'user' }, process.env.JWT_SECRET, { expiresIn: '1h' });
...
// Serve all frontend HTML files using clean URLs
app.get('*', (req, res) => {
    const filePath = path.join(__dirname, 'public', req.path.endsWith('/') ? 'index.html' : req.path);
